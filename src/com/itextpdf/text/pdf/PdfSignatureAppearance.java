...
//                sigField.setWidget(new Rectangle(0, 0), null);
            if(isInvisible()) { //Si la firma es INVISIBLE agrega sólo el campo de la firma
                sigField.setWidget(new Rectangle(0, 0), null);
                
            } else if (pagen==0) { //Si la firma es VISIBLE y página=0 tonces creará los objetos visibles en cada página
                int pages = writer.reader.getNumberOfPages();
                for (int i = 1; i <= pages; i++) {
                    PdfFormField field = PdfFormField.createEmpty(writer);
                    this.page = i;
                    pagen= i;
                    String nombreCampo = name + "-" + i;
                    field.setWidget(getPageRect(), new PdfName(nombreCampo));
            ...
            } else { //Si la firma es VISIBLE y página!=0 tonces creará los objetos visibles en la página indicada
                sigField.setWidget(getPageRect(), null);
            }
